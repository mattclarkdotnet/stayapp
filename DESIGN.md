...
- For untitled multi-window snapshot sets (common with WindowServer fallback), if AX
  still exposes fewer windows than expected, app restore is deferred and retried instead
  of forcing partial/wrong matches.
